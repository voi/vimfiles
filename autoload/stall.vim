...
function! stall#command_open(mods, args) "{{{
  ...
  nnoremap <buffer> <silent> i :call stall#api_handle_key('_do_filtering')<CR>
  nnoremap <buffer> <silent> I :call stall#api_handle_key('_do_filtering_fuzzy')<CR>
  nnoremap <buffer> <silent> Q :call stall#api_handle_key('_switch_no_quit')<CR>
...
function! stall#api_handle_key(name) "{{{
  "
...
function! stall#api_get_buffer_opener() "{{{
  return {
...
function! stall#api_get_file_opener() "{{{
  return {
...
function! g:stall_sources.files._collection(context, item, flags) dict "{{{
  return sort(map(globpath(a:context.root, '*', 0, 1),
      ...
      \ { i1, i2 -> i1.text ==# i2.text ? 0 : i1.text ># i2.text ? 1 : -1 })
endfunction "}}}

function! g:stall_sources.files._on_ready(context, item, flags) dict "{{{
  nnoremap <buffer> <silent> <CR> :call stall#api_handle_key('enter')<CR>
  ...
  nnoremap <buffer> <silent> <S-CR> :call stall#api_handle_key('enter_with_quit')<CR>
  nnoremap <buffer> <silent> -    :call stall#api_handle_key('up')<CR>
...
" ********
let g:stall_sources.history = extend({}, stall#api_get_file_opener())
